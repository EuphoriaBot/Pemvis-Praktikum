...
Module DataModule
    ...
    Public Function GetAllTanaman() As DataTable
        ...
        Try
            Dim query As String = "SELECT idTanaman, namaTanaman, namaJenis, lokasiLahan, " & "hargaBeli, hargaJual, (hargaJual - hargaBeli) AS keuntungan, " & "keterangan, fotoPath " & "FROM tbtanaman as T LEFT JOIN tbjenis as J on J.idJenis = T.idJenis ORDER BY idTanaman ASC "
    ...
    Public Function SearchTanaman(keyword As String) As DataTable
        Dim dt As New DataTable()
        Try
            Dim kondisi As String = "WHERE namaTanaman LIKE @keyword " & "OR namaJenis LIKE @keyword " & "OR lokasiLahan LIKE @keyword"
            Dim query As String = "SELECT idTanaman, namaTanaman, namaJenis, lokasiLahan, " & "hargaBeli, hargaJual, (hargaJual - hargaBeli) AS keuntungan " & "FROM tbtanaman as T LEFT JOIN tbjenis as J on J.idJenis = T.idJenis " & kondisi & " ORDER BY idTanaman ASC"
            Using conn As MySqlConnection = GetConnection()
                Using da As New MySqlDataAdapter(query, conn)
    ...
    Public Function SimpanTanaman(nama As String, jenis As String, lokasi As String, hargaBeli As Long, hargaJual As Long, keterangan As String, fotoPath As String) As Boolean
        Try
            Dim query As String = "INSERT INTO tbtanaman (namaTanaman, idJenis, lokasiLahan, " & "hargaBeli, hargaJual, keterangan, fotoPath) " & "VALUES (@nama, @jenis, @lokasi, @hargaBeli, @hargaJual, @ket, @foto)"
            Using conn As MySqlConnection = GetConnection()
                conn.Open()
    ...
    Public Function UbahTanaman(id As Integer, nama As String, jenis As String, lokasi As String, hargaBeli As Long, hargaJual As Long, keterangan As String, fotoPath As String) As Boolean
        Try
            Dim query As String = "UPDATE tbtanaman SET namaTanaman=@nama, idJenis=@jenis, " & "lokasiLahan=@lokasi, hargaBeli=@hargaBeli, hargaJual=@hargaJual, " & "keterangan=@ket, fotoPath=@foto WHERE idTanaman=@id"
            Using conn As MySqlConnection = GetConnection()
                conn.Open()
                Using cmd As New MySqlCommand(query, conn)
